...
bool SymbolTable::insert(const string& name, const string& type, const std::string &varDataType, const std::string &functionReturnType, int functionArgumentNumber, std::vector<std::string>parameterTypes) {
    if (currentScope == nullptr) {
        // *out << "\tNo current scope" << endl;
        return false;
    }
    
    return currentScope->insert(name, type, varDataType, functionReturnType, functionArgumentNumber, parameterTypes);
}
