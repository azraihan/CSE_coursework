...
PRINTF  : 'printf' { writeIntoLexLogFile("Line# " + std::to_string(getLine()) + ": Token <PRINTF> Lexeme " + getText()); }; // added printf
...
ADDASSIGNOP : '+=' { writeIntoLexLogFile("Line# " + std::to_string(getLine()) + ": Token <ADDASSIGNOP> Lexeme " + getText()); }; //added add and assign operator
...
///////
ERROR_CHAR: . {
    writeIntoLexLogFile("Line# " + std::to_string(getLine()) + ": Unrecognized character " + getText());
};
