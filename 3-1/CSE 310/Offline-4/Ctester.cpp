#include <iostream>
#include <fstream>
#include <string>
#include "antlr4-runtime.h"
#include "C8086Lexer.h"
#include "C8086Parser.h"

#include "SymbolTable/2105009_SymbolTable.h"
#include "optimizer/peephole_optimizer.h"

using namespace antlr4;
using namespace std;

ofstream parserLogFile; // global output stream
// ofstream errorFile; // global error stream
ofstream lexLogFile; // global lexer log stream

////////////////////////////////////////
// assembly file for the generated code
ofstream assemblyFile;
////////////////////////////////////////

// int syntaxErrorCount;


//////////////////// declaring symbol table////////////////////
SymbolTable symbolTable = SymbolTable(7, parserLogFile);
///////////////////////////////////////////////////////////////

int main(int argc, const char* argv[]) {
    if (argc < 2) {
        cerr << "Usage: " << argv[0] << " <input_file>" << endl;
        return 1;
    }

    // ---- Input File ----
    ifstream inputFile(argv[1]);
    if (!inputFile.is_open()) {
        cerr << "Error opening input file: " << argv[1] << endl;
        return 1;
    }

    string outputDirectory = "output/";
    string parserLogFileName = outputDirectory + "parserLog.txt";
    // string errorFileName = outputDirectory + "errorLog.txt";
    string lexLogFileName = outputDirectory + "lexerLog.txt";

    string assemblyFileName = outputDirectory + "code.asm";

    // create output directory if it doesn't exist
    system(("mkdir -p " + outputDirectory).c_str());

    // ---- Output Files ----
    parserLogFile.open(parserLogFileName);
    if (!parserLogFile.is_open()) {
        cerr << "Error opening parser log file: " << parserLogFileName << endl;
        return 1;
    }

    // errorFile.open(errorFileName);
    // if (!errorFile.is_open()) {
    //     cerr << "Error opening error log file: " << errorFileName << endl;
    //     return 1;
    // }

    lexLogFile.open(lexLogFileName);
    if (!lexLogFile.is_open()) {
        cerr << "Error opening lexer log file: " << lexLogFileName << endl;
        return 1;
    }


    ////////////////////////////////////////////////////////////////////////////////////
    assemblyFile.open(assemblyFileName);
    if (!assemblyFile.is_open()) {
        cerr << "Error opening assembly file: " << assemblyFileName << endl;
        return 1;
    }
    ////////////////////////////////////////////////////////////////////////////////////

   
    // ---- Parsing Flow ----
    ANTLRInputStream input(inputFile);
    C8086Lexer lexer(&input);
    CommonTokenStream tokens(&lexer);
    C8086Parser parser(&tokens);

    // to avoid the default error listener and use custom error handling
    parser.removeErrorListeners();

    // start parsing at the 'start' rule
    parser.start();

    // clean up
    inputFile.close();
    parserLogFile.close();
    // errorFile.close();
    lexLogFile.close();

    // optimizer
    peepholeOptimize("output/code.asm");


    ////////////////////////////////////////////////////////////////////////////////////
    assemblyFile.close();
    ////////////////////////////////////////////////////////////////////////////////////

    cout << "Parsing and code generation completed. Check the output files for details." << endl;
    return 0;
}
